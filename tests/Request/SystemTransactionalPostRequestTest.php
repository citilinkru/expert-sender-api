...
class SystemTransactionalPostRequestTest extends \PHPUnit_Framework_TestCase
{
    public function testRequestCompileValidXml()
    {
        $request = new SystemTransactionalPostRequest(
            12,
    ...
    {
        $request = new SystemTransactionalPostRequest(12, Receiver::createWithId(12, 25));
        $this->assertEquals(
            '<Data><Receiver><Id>12</Id><ListId>25</ListId></Receiver></Data>',
            $request->toXml()
        );

        $request2 = new SystemTransactionalPostRequest(12, Receiver::createWithEmail('mail@mail.com'));
        $this->assertEquals(
            '<Data><Receiver><Email>mail@mail.com</Email></Receiver></Data>',
            $request2->toXml()
        );

        $request3 = new SystemTransactionalPostRequest(12, Receiver::createWithEmailMd5(md5('mail@mail.com')));
        $this->assertEquals(
